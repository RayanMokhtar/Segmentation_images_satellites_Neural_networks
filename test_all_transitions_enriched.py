# -*- coding: utf-8 -*-
"""
test_all_transitions_enriched.py

Script d'évaluation des transitions dans le jeu de test enrichi :
...
try:
    ...
    if 'historique_region' not in df_peek.columns:
        print("ATTENTION: La colonne 'historique_region' n'est pas présente dans le dataset de test!")
        # Supprimer de la liste des features si non présente
    ...
    try:
        ...
        print(f"Jeu de test enrichi chargé sans filtrage: {len(df_test)} lignes")
        ...
        print(f"Jeu de test standard chargé sans filtrage: {len(df_test)} lignes")
...
try:
    # Essayer d'abord le modèle enrichi
    model = load_model('best_model_enriched.keras')
    print("Modèle enrichi chargé avec succès: best_model_enriched.keras")
except Exception as e:
    print(f"Erreur lors du chargement du modèle best_model_enriched.keras: {str(e)}")
    try:
        # Essayer le modèle standard
        model = load_model('best_model.keras')
        print("Modèle standard chargé avec succès: best_model.keras")
    except Exception as e2:
        print(f"Erreur lors du chargement du modèle best_model.keras: {str(e2)}")
        ...
        if model_files:
            ...
            try:
                ...
                print(f"Erreur lors du chargement du modèle alternatif: {str(e3)}")
...
for region, grp in df_test.groupby('chemin_directory'):
    ...
    # 10c. Extraction de la séquence des L jours
...
if len(y_trues) > 0:
    ...
    print("\nAUCUNE TRANSITION TROUVÉE DANS LE JEU DE TEST!")
