# -*- coding: utf-8 -*-
"""
test_transitions_enriched.py

Script d'évaluation des transitions dans le jeu de test avec le modèle enrichi :
1. Charger directement test_dataset_enriched.csv (données de test déjà exportées)
...
# Météo / topo numériques + historique_region + soil_type + label + date + region
# Attention: les coordonnées lat/long ont été retirées, et historique_region a été ajouté
numeric_feats = [
    'tempmax','tempmin','temp','feelslikemax','feelslikemin','feelslike',
    'dew','humidity','precipprob','precipcover',
    'windspeed','winddir','pressure','cloudcover','visibility',
    'elevation','historique_region'
]
cat_feats = ['soil_type']
all_feats = numeric_feats + cat_feats

# --- 3. Chargement du jeu de test directement depuis test_dataset_enriched.csv ---
print("Chargement du jeu de test enrichi...")
try:
    ...
    # Vérifier si la colonne historique_region est présente
    has_historique = 'historique_region' in df_peek.columns
    if not has_historique:
        print("ATTENTION: La colonne 'historique_region' n'est pas présente dans le dataset de test!")
        print("Cette colonne est requise pour le modèle enrichi.")
        # Supprimer de la liste des features
        numeric_feats = [f for f in numeric_feats if f != 'historique_region']
    
    # Vérifier si les colonnes de latitude/longitude sont présentes (elles ne doivent pas être utilisées)
    coord_cols = ['latitude_centroid', 'longitude_centroid']
    for col in coord_cols:
        if col in df_peek.columns:
            print(f"ATTENTION: La colonne '{col}' est présente mais ne doit pas être utilisée avec le modèle enrichi.")
    ...
    # Ajouter la colonne historique_region si elle n'existe pas
    if not has_historique:
        df_test['historique_region'] = 0
        print("Une colonne 'historique_region' factice a été ajoutée avec des valeurs à zéro.")
        # Réajouter à la liste des features
        if 'historique_region' not in numeric_feats:
    ...
    print(f"Erreur lors du chargement du fichier test_dataset_enriched.csv: {str(e)}")
    ...
    try:
        ...
        print("Fichier test_dataset_enriched.csv non trouvé, tentative avec test_dataset.csv...")
        df_test = pd.read_csv(
            './test_dataset.csv',
            parse_dates=['date'],
            dayfirst=True
    ...
    # Vérifier et ajouter la colonne historique_region si nécessaire
    if 'historique_region' not in df_test.columns:
        df_test['historique_region'] = 0
        print("Une colonne 'historique_region' factice a été ajoutée avec des valeurs à zéro.")
        # Réajouter à la liste des features
        if 'historique_region' not in numeric_feats:
...
try:
    model = load_model('./modele_inondation_enriched.keras')
    print("Modèle chargé avec succès: modele_inondation_enriched.keras")
except Exception as e:
    print(f"Erreur lors du chargement du modèle modele_inondation_enriched.keras: {str(e)}")
    try:
        model = load_model('./best_model_enriched.keras')
        print("Modèle chargé avec succès: best_model_enriched.keras")
    except Exception as e:
        print(f"Erreur lors du chargement du modèle best_model_enriched.keras: {str(e)}")
...
# --- 9. Afficher la structure du modèle chargé pour vérification ---
print("\nRésumé du modèle chargé:")
model.summary()
print(f"Forme d'entrée attendue: {model.input_shape}")
